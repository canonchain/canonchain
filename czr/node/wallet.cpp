...
std::shared_ptr<czr::block> czr::wallet::send_action (czr::account const & from_a, czr::account const & to_a, czr::uint128_t const & amount_a, bool generate_work_a, boost::optional<std::string> id_a)
{
	czr::publish message;
	...
	bool cached_block = false;
	{
		...
		if (!error && message.block == nullptr)
		{
			if (store.valid_password (transaction))
			{
				...
				{
					auto balance (node.ledger.account_balance (transaction, from_a));
					if (!balance.is_zero () && balance >= amount_a)
					{
						...
						message.block.reset(new czr::block(from_a, to_a, amount_a, previous, parents, witness_list_block, witness_list, last_summary, last_summary_block, data, prv, from_a, generate_work_a ? work_fetch(transaction, from_a, previous) : 0));
