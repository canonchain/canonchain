...
void czr::witness::check_and_witness()
{
	...
	{
		std::weak_ptr<czr::witness> this_l(shared_from_this());
	...
	else
	{
		...
		if (has_unstable_non_witness_block)
		{
			...
			node.alarm.add(std::chrono::steady_clock::now() + random_period, [this_l]() {
				if (auto this_s = this_l.lock())
				{
					this_s->do_witness_before_threshold();
				}
			});
		}
	}
...
void czr::witness::do_witness()
{
	...
	std::weak_ptr<czr::witness> this_w(shared_from_this());
	node.wallet.send_async(from, to, amount, data, "", [from, this_w](czr::send_result result) {
		if (auto this_l = this_w.lock())
		{
			...
			case czr::send_result_codes::validate_error:
			{
				...
				std::chrono::milliseconds random_period(random_pool.GenerateWord32(max_do_witness_interval/2, max_do_witness_interval));
...
uint64_t const czr::witness::threshold_distance(10);
