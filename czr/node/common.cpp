...
czr::frontier_req::frontier_req() :
	message(czr::message_type::frontier_req)
{
}

bool czr::frontier_req::deserialize(czr::stream & stream_a)
{
	auto result(read_header(stream_a, version_max, version_using, version_min, type, extensions));
	assert(!result);
	assert(czr::message_type::frontier_req == type);
	if (!result)
	{
		assert(type == czr::message_type::frontier_req);
...
void czr::frontier_req::serialize(czr::stream & stream_a)
{
	write_header(stream_a);
...
void czr::frontier_req::visit(czr::message_visitor & visitor_a) const
{
	visitor_a.frontier_req(*this);
}

bool czr::frontier_req::operator== (czr::frontier_req const & other_a) const
{
	return start == other_a.start && age == other_a.age && count == other_a.count;
}

czr::bulk_pull::bulk_pull() :
	message(czr::message_type::bulk_pull)
{
}

void czr::bulk_pull::visit(czr::message_visitor & visitor_a) const
{
	visitor_a.bulk_pull(*this);
}

bool czr::bulk_pull::deserialize(czr::stream & stream_a)
{
	auto result(read_header(stream_a, version_max, version_using, version_min, type, extensions));
	assert(!result);
	assert(czr::message_type::bulk_pull == type);
	if (!result)
	{
		assert(type == czr::message_type::bulk_pull);
...
void czr::bulk_pull::serialize(czr::stream & stream_a)
{
	write_header(stream_a);
...
czr::bulk_pull_blocks::bulk_pull_blocks() :
	message(czr::message_type::bulk_pull_blocks)
{
}

void czr::bulk_pull_blocks::visit(czr::message_visitor & visitor_a) const
{
	visitor_a.bulk_pull_blocks(*this);
}

bool czr::bulk_pull_blocks::deserialize(czr::stream & stream_a)
{
	auto result(read_header(stream_a, version_max, version_using, version_min, type, extensions));
	assert(!result);
	assert(czr::message_type::bulk_pull_blocks == type);
	if (!result)
	{
		assert(type == czr::message_type::bulk_pull_blocks);
...
void czr::bulk_pull_blocks::serialize(czr::stream & stream_a)
{
	write_header(stream_a);
...
czr::bulk_push::bulk_push() :
	message(czr::message_type::bulk_push)
{
}

bool czr::bulk_push::deserialize(czr::stream & stream_a)
{
	auto result(read_header(stream_a, version_max, version_using, version_min, type, extensions));
	assert(!result);
	assert(czr::message_type::bulk_push == type);
	return result;
}

void czr::bulk_push::serialize(czr::stream & stream_a)
{
	write_header(stream_a);
}

void czr::bulk_push::visit(czr::message_visitor & visitor_a) const
{
