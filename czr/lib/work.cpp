...
bool czr::work_validate(czr::block_hash const & root_a, uint64_t work_a)
{
	return czr::work_value(root_a, work_a) < czr::work_pool::publish_threshold;
}

bool czr::work_validate(czr::block const & block_a)
{
	return work_validate(block_a.root(), block_a.block_work());
}

uint64_t czr::work_value(czr::block_hash const & root_a, uint64_t work_a)
{
	uint64_t result;
	blake2b_state hash;
...
czr::work_pool::work_pool(unsigned max_threads_a, std::function<boost::optional<uint64_t>(czr::uint256_union const &)> opencl_a) :
	ticket(0),
	done(false),
	opencl(opencl_a)
{
	static_assert (ATOMIC_INT_LOCK_FREE == 2, "Atomic int needed");
	auto count(czr::czr_network == czr::czr_networks::czr_test_network ? 1 : std::max(1u, std::min(max_threads_a, std::thread::hardware_concurrency())));
	for (auto i(0); i < count; ++i)
	{
		auto thread(std::thread([this, i]() {
			czr::work_thread_reprioritize();
			loop(i);
		}));
...
void czr::work_pool::loop(uint64_t thread)
{
	// Quick RNG for work attempts.
	xorshift1024star rng;
	czr::random_pool.GenerateBlock(reinterpret_cast<uint8_t *> (rng.s.data()), rng.s.size() * sizeof(decltype (rng.s)::value_type));
	uint64_t work;
	uint64_t output;
	blake2b_state hash;
	...
	while (!done || !pending.empty())
	{
		...
		if (!empty)
		{
			...
			while (ticket == ticket_l && output < czr::work_pool::publish_threshold)
			{
				...
				while (iteration && output < czr::work_pool::publish_threshold)
				{
					work = rng.next();
			...
			if (ticket == ticket_l)
			{
				...
				assert(output >= czr::work_pool::publish_threshold);
...
void czr::work_pool::cancel(czr::uint256_union const & root_a)
{
...
void czr::work_pool::generate(czr::uint256_union const & root_a, std::function<void(boost::optional<uint64_t> const &)> callback_a)
{
...
uint64_t czr::work_pool::generate(czr::uint256_union const & hash_a)
{
