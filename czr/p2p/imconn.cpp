...
CImConn::~CImConn()
{
	printf("CImConn::~CImConn, handle=%d ", m_handle);
}
...
int CImConn::Send(void* data, int len)
{
	...
	if (remain > 0)
	{
		...
		m_busy = true;
		printf("send busy, remain=%d ", m_out_buf.GetWriteOffset());
	}
	else
	{
		OnWriteCompelete();
...
void CImConn::OnWrite()
{
	...
	printf("onWrite, remain=%d ", m_out_buf.GetWriteOffset());
}
