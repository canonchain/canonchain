...
int CBaseSocket::Listen(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	if (ret == SOCKET_ERROR)
	{
		printf("bind failed, err_code=%d", _GetErrorCode());
	...
	if (ret == SOCKET_ERROR)
	{
		printf("listen failed, err_code=%d", _GetErrorCode());
		closesocket(m_socket);
		return NETLIB_ERROR;
	}
...
net_handle_t CBaseSocket::Connect(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	if (m_socket == INVALID_SOCKET)
	{
		printf("socket failed, err_code=%d", _GetErrorCode());
	...
	{
		printf("connect failed, err_code=%d", _GetErrorCode());
		closesocket(m_socket);
		return NETLIB_INVALID_HANDLE;
	}
...
void CBaseSocket::_SetNonblock(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		printf("_SetNonblock failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetReuseAddr(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		printf("_SetReuseAddr failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetNoDelay(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		printf("_SetNoDelay failed, err_code=%d", _GetErrorCode());
	}
}
